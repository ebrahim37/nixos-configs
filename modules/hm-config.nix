...
in
{
	...
	programs = {
		bash = {
			enable = true;
			initExtra = ''
				if [ -z "''${TMUX:-}" ] && { [ -n "''${SSH_TTY:-}" ] || [ "''${TERM:-}" = foot ]; }; then
					...
					client_tmux_session="client-$$"
					tmux new-session -d -s "$client_tmux_session" -t main
					tmux new-window -t "$client_tmux_session:" -c "$HOME" \
						"bash -l; tmux kill-session -t =$client_tmux_session"

					cleanup_client_tmux_session() {
						tmux kill-session -t "=$client_tmux_session" 2>/dev/null || true
					}
					trap cleanup_client_tmux_session EXIT HUP INT TERM
					tmux attach-session -t "=$client_tmux_session"
					exit
				fi
